...
import { useTheme, icons } from './themeContext';
...
const ThemeSchemaSwitch: React.FC<ThemeSwitchProps> = memo(({ showTooltip = true, tooltipPlacement = 'bottom', ...props }) => {
    const { t } = useTranslation();
    const { darkMode, toggleThemeScheme, themeScheme } = useTheme();
    ...
    const toggleDark: ButtonProps["onClick"] = event => {
        ...
        const transition = document.startViewTransition(toggleThemeScheme.bind(this));
        ...
        transition.ready.then(() => {
            const clipPath = [`circle(0px at ${rect.x}px ${rect.y}px)`, `circle(${endRadius}px at ${rect.x}px ${rect.y}px)`];
